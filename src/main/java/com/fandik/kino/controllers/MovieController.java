...
public class MovieController {
    ...
    public ResponseEntity<MovieEntity> getMovieById(@PathVariable(ID) Long id) {
        ...
        return movie.map(film -> new ResponseEntity<>(film, HttpStatus.OK))
                .orElse(new ResponseEntity<>(HttpStatus.NOT_FOUND));
    }

    @PostMapping
    public ResponseEntity<MovieEntity> save(@RequestBody MovieEntity movieEntity, @AuthenticationPrincipal UserEntity userEntity) {
        if (userEntity !=null) {
